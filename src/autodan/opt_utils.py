...
def forward(*, model, input_ids, attention_mask, target_slices, batch_size=512):
    logits = []
    for i in range(0, input_ids.shape[0], batch_size):

        batch_input_ids = input_ids[i:i + batch_size]
        batch_target_slices = target_slices[i:i + batch_size]
        batch_loss_slices = [slice(target_slice.start - 1, target_slice.stop - 1) for target_slice in batch_target_slices]
        if attention_mask is not None:
            batch_attention_mask = attention_mask[i:i + batch_size]
        else:
            batch_attention_mask = None
        batch_logits = model(input_ids=batch_input_ids, attention_mask=batch_attention_mask).logits
        batch_logits = [batch_logits[idx, batch_loss_slices[idx], :] for idx in range(batch_logits.shape[0])]
...
def gpt_mutate(sentence, API_key=None):
    ...
    while not received:
        try:
            ...
            if error == openai.InvalidRequestError:  # something is wrong: e.g. prompt too long
...
def get_score_autodan(tokenizer, conv_template, instruction, target, model, device, test_controls=None, crit=None,
    ...
    while not flag:
        try:
            logits = forward(model=model, input_ids=input_ids_tensor, attention_mask=attn_mask, target_slices=target_slices, batch_size=batch_size)
            flag = True
    ...
    for idx, target_slice in enumerate(target_slices):
        logits_slice = logits[idx].unsqueeze(0).transpose(1, 2).cuda()
